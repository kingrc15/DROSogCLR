...
parser.add_argument(
    "--t", default="", type=float, help="path to moco pretrained checkpoint"
)
...
def main(pretrained=None, gpu=0):
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        save_checkpoint(
            ...
            is_best,
            filename=os.path.join(save_root_path, logdir, "checkpoint.pth.tar"),
            save_path=os.path.join(save_root_path, logdir),
        )
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        wandb.log({"epoch": epoch, "train acc1": acc1[0], "train acc5": acc5[0]})
...
def validate(val_loader, model, criterion, args):
    ...
    with torch.no_grad():
        ...
        wandb.log({"valid top1": top1.avg, "valid top5": top5.avg})
...
def sanity_check(state_dict, pretrained_weights, linear_keyword):
    ...
    for k in list(state_dict.keys()):
        ...
        k_pre = (
            "module.base_encoder." + k[len("module.") :]
            if k.startswith("module.")
            else "module.base_encoder." + k
        )
